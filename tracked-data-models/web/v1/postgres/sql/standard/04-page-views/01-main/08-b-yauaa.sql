...
CREATE TABLE IF NOT EXISTS {{.scratch_schema}}.pv_addon_yauaa{{.entropy}} (
  ...
  INSERT INTO {{.scratch_schema}}.pv_addon_yauaa{{.entropy}} (
    ...
    WHERE ya.root_tstamp >= (select max(run_id) from {{.output_schema}}.pv_last_success_run_id{{.entropy}})
      AND ya.root_tstamp <= (select max(run_id) from {{.scratch_schema}}.metadata_run_id{{.entropy}})
  );
