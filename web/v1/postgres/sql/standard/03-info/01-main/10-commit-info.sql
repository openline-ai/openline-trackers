...
INSERT INTO {{.output_schema}}.app_info{{.entropy}} (id, tenant, name_tracker, app_id, platform, updated_on)
    ...
    FROM    {{.scratch_schema}}.app_info_this_run{{.entropy}} a left join
...
SET
    ...
    last_visit = scratchTable.last_visit
from {{.output_schema}}.page_info{{.entropy}} outputTable inner join
    {{.scratch_schema}}.page_info_this_run{{.entropy}} scratchTable
...
INSERT INTO {{.output_schema}}.page_info{{.entropy}} (id, tenant, name_tracker, app_id, page_url, page_title, updated_on, last_visit)
    SELECT  a.id, a.tenant, a.name_tracker, a.app_id, a.page_url, a.page_title, a.updated_on, a.last_visit
    FROM    {{.scratch_schema}}.page_info_this_run{{.entropy}} a left join
